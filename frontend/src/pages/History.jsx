...
export default function History() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getHistory(getSessionId(), 10);
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Your last 10 generations on this device.
          </p>
        </div>
          ...
          {items.map((it) => (
            <li key={it.generation_id}>
              <Link
                ...
                className="card flex flex-col gap-3 p-4 transition hover:border-brand-300/30 hover:bg-white/[0.07] sm:flex-row sm:items-center sm:justify-between"
              >
