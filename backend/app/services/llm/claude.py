...
async def refine_terraform(
    *,
    ...
    """Apply a change to EXISTING Terraform files and return the full updated set.

    Works regardless of how the original was created (text or diagram) because it
    edits the generated files directly rather than re-running from a description.
    """
    settings = get_settings()
    if not settings.ANTHROPIC_API_KEY:
        raise ClaudeServiceError("ANTHROPIC_API_KEY is not configured.")
    ...
    current = "\n\n".join(
        f"=== {name} ===\n{files.get(name, '')}"
        for name in ("main.tf", "variables.tf", "outputs.tf", "providers.tf")
    )
    user_text = (
        "You are refining an EXISTING Terraform configuration. Apply the requested change "
        "and return the COMPLETE updated set of all four files via the submit_terraform_v1 "
        "tool (return full files, not a diff). Keep everything else intact, preserve "
        "<REPLACE_*> placeholders and explanatory comments, and follow the same security "
    ...
    try:
        ...
        logger.exception("Claude refine call failed")
        raise ClaudeServiceError(f"Claude refine call failed: {exc}") from exc
    ...
    missing = [k for k, v in new_files.items() if not v.strip()]
    if missing:
        raise ClaudeServiceError(f"Claude returned empty Terraform files: {missing}")
