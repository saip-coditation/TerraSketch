...
16a. For aws_instance launched into a VPC subnet, attach security groups via `vpc_security_group_ids = [aws_security_group.web.id]` (security-group *IDs*) — NEVER `security_groups = [...]` (that is the EC2-Classic groupName parameter and fails with "InvalidParameterCombination: The parameter groupName cannot be used with the parameter subnet"). Reference the group by `.id`, never by a hardcoded name string. Use `filter` (not `filters`) blocks inside data "aws_ami".
...
16e. On aws_db_instance the database name argument is `db_name` (NOT `name`, which was removed) — use `db_name = var.db_name`. `identifier` is the instance id. The DB subnet group argument is `db_subnet_group_name` (NOT `subnet_group_name`, which belongs to aws_elasticache_cluster).
...
16k. Auto Scaling: NEVER use aws_launch_configuration — it is deprecated and blocked on all new AWS accounts ("UnsupportedOperation: The Launch Configuration creation operation is not available in your account"). Always use aws_launch_template, and reference it from aws_autoscaling_group via a `launch_template { id = ..., version = "$Latest" }` block. On the ASG, set instance/volume tags with `tag { key = ..., value = ..., propagate_at_launch = true }` blocks (NOT a `tags = [...]` argument)."""
