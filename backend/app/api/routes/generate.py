...
@router.post(
    "/generation/{generation_id}/refine",
    response_model=GenerateResponse,
    summary="Modify a generation's Terraform from a plain-English instruction",
)
async def refine_generation(
    ...
    if not files:
        raise HTTPException(status_code=400, detail="Generation has no files to refine")

    # Reuse the same provider-consistent LLM the deploy self-heal uses (GPT-4o via
    # the Azure layer), so refined code matches what actually deploys.
    from app.api.routes.review import _call_llm

    system = (
        "You are a Terraform expert. Modify the given Terraform files to satisfy the user's "
        "instruction, changing ONLY what's needed and keeping the rest intact and valid "
        "(AWS provider ~> 5.0; v5 idioms; every variable keeps a working default). "
        'Return ONLY a JSON object with keys "main.tf", "variables.tf", "outputs.tf", '
        '"providers.tf", each mapping to the COMPLETE updated file content. No prose.'
    )
    blob = "\n\n".join(f"=== {n} ===\n{c}" for n, c in files.items() if (c or "").strip())
    user = f"Instruction:\n{payload.instruction}\n\nCurrent files:\n{blob}"

    try:
        raw = (await asyncio.to_thread(_call_llm, system, user) or "").strip()
    ...
    for k in ("main.tf", "variables.tf", "outputs.tf", "providers.tf"):
        ...
        if isinstance(v, str) and v.strip():
            ...
            changed = True
    if not changed:
        raise HTTPException(status_code=502, detail="Refine produced no usable changes; try rephrasing.")

    record.generated_files = new_files
    record.security_warnings = scan_generated_files(new_files) + scan_best_practices(new_files)
    db.add(record)
    db.commit()
    db.refresh(record)
