...
- Size compute/database/cache instance types and autoscaling/replica counts to the given "Scale tier": `small` = single instance, burstable/free-tier-class sizing, no autoscaling; `mid` = right-sized general-purpose instances, 2+ AZ redundancy, modest autoscaling range; `high` = larger instance classes, multi-AZ/read-replicas by default, autoscaling sized for real load. Put the concrete sizes/counts you chose in each resource's `args`.
...
- ALWAYS fill `args` with a concrete, working default for every configurable value (instance size, multi-AZ, bucket public access, autoscaling max) — never leave a choice unmade — so generation can complete even if the user answers no questions.
- For the resources where that default is genuinely a judgment call not clearly implied by the diagram or scale tier, ALSO add an entry to `clarifying_questions` with 2-4 concrete options so the user can override your default. Prefer questions across these dimensions (not just sizing): (a) SIZE — instance class / DB or cache node type; (b) SCALING — desired/min/max count, autoscaling or Multi-AZ/replicas; (c) STORAGE — S3 versioning, encryption, public-access, allocated storage / lifecycle; (d) OBSERVABILITY — whether to add CloudWatch logs, metrics/alarms, and log retention days; (e) BACKUPS — retention period / deletion protection. Each option's `value` must be the literal value written to the resource's `target_field` `args` (e.g. field `multi_az` -> value `true`/`false`; field `retention_in_days` -> `7`/`30`/`90`; field `versioning` -> `Enabled`/`Disabled`). Keep every option working so generation still succeeds if unanswered. Cap at the 5 most cost/security/operability-impactful questions — skip cosmetic ones.
